...
class WarehouseExplore(Node):
	...
	def __init__(self):
		...
		self.yaw = 0.0
		self.finding_qr_code = False
		self.orienting_towards_shelf = False
		self.shelfs_published_count = 0
		self.current_shelf_angle = self.initial_angle
		self.done = False
	...
	def pose_callback(self, message):
		...
		orientaion = message.pose.pose.orientation
		q = [orientaion.x,orientaion.y,orientaion.z,orientaion.w]
		r = R.from_quat(q)
		yaw = r.as_euler('xyz', degrees=False)[2]
		self.buggy_yaw_radian = yaw
	...
	def shelf_data_publish(self):
		shelf_data = self.shelf_objects_curr
		if(sum(shelf_data.object_count)==6 and shelf_data.qr_decoded!='' and int(shelf_data.qr_decoded[0])>self.shelfs_published_count):
			self.publisher_shelf_data.publish(shelf_data)
			self.shelfs_published_count += 1
			self.current_shelf_angle = float(shelf_data.qr_decoded[2:7])
			self.finding_qr_code = False
			...
			if(self.shelfs_published_count == self.shelf_count):
				self.get_logger().info("All Shelfs Done")
				self.done = True
	...
	def global_map_callback2(self, message):
		
		if not self.goal_completed:
			return
		
		if(self.finding_qr_code or self.orienting_towards_shelf or self.done):
			return
		
		theta_total =  self.current_shelf_angle * (math.pi / 180)
		
		x_goal = self.prev_goal_x + 1 * math.cos(theta_total)
		y_goal = self.prev_goal_y + 1 * math.sin(theta_total)

		self.prev_goal_x = x_goal
		self.prev_goal_y = y_goal

		goal_pose = self.create_goal_from_world_coord(x_goal,y_goal)
	...
	def find_qr_code(self):
		...
		xi = self.buggy_pose_x
		yi = self.buggy_pose_y
		thetai = self.buggy_yaw_radian

		xf = xi + x_rel*math.cos(thetai) - y_rel*math.sin(thetai)
		yf = yi + x_rel*math.sin(thetai) + y_rel*math.cos(thetai)

		thetaf = thetai - math.pi/2

		goal_pose = self.create_goal_from_world_coord(xf,yf,thetaf)
		self.send_goal_from_world_pose(goal_pose)
